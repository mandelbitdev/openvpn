...
multi_process_incoming_dco(struct multi_context *m)
{
    ...
    if ((peer_id < m->max_clients) && (m->instances[peer_id]))
    {
        ...
        if (dco->dco_message_type == OVPN_CMD_DEL_PEER || dco->dco_message_type == OVPN_V2_CMD_DEL_PEER)
        {
            process_incoming_del_peer(m, mi, dco);
        }
        else if (dco->dco_message_type == OVPN_CMD_SWAP_KEYS || dco->dco_message_type == OVPN_V2_CMD_SWAP_KEYS)
        {
