...
multi_io_set_global_rw_flags(struct multi_context *m, struct multi_instance *mi)
{
    if (mi)
    {
        ...
        socket_set(mi->context.c2.link_sockets[0],
                   ...
                   !proto_is_dgram(mi->context.c2.link_sockets[0]->info.proto) ? &mi->ev_arg : &mi->context.c2.link_sockets[0]->ev_arg,
                   !proto_is_dgram(mi->context.c2.link_sockets[0]->info.proto) ? &mi->tcp_rwflags : NULL);
    }
}
            ...
            switch (ev_arg->type)
            {
                ...
                case EVENT_ARG_LINK_SOCKET:
                    ...
                    else
                    {
                        ...
                        mi = m->pending;
                        /* Mimic TCP and monitor and/or handle events that
                         * are triggered in succession by the first one
