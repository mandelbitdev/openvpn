...
static int
ovpn_dco_iface_new(struct tuntap *tt, int flags, const char *dev)
{
    int ret = -EMSGSIZE;
    struct nl_msg *nl_msg = nlmsg_alloc();
    genlmsg_put(nl_msg, 0, 0, tt->dco.ovpn_dco_id, 0, flags, OVPN_CMD_DEV_NEW, 0);
...
static int
ovpn_dco_iface_del(struct tuntap *tt, const char *dev)
{
    int ret = -EMSGSIZE;
    struct nl_msg *nl_msg = nlmsg_alloc();
    genlmsg_put(nl_msg, 0, 0, tt->dco.ovpn_dco_id, 0, 0, OVPN_CMD_DEV_DEL, 0);
    if (!tt->dco.ifindex)
    {
...
open_tun_dco(struct tuntap *tt, openvpn_net_ctx_t *ctx, const char *dev)
{
    ...
    ret = ovpn_dco_iface_new(tt, 0, dev);
    if (ret < 0)
    {
...
close_tun_dco(struct tuntap *tt, openvpn_net_ctx_t *ctx)
{
    ...
    ovpn_dco_iface_del(tt, tt->actual_name);
            ...
            dco_key_slot_t slot)
{
    ...
    keyconf = nla_nest_start(nl_msg, OVPN_A_KEYCONF);
    NLA_PUT_U32(nl_msg, OVPN_A_KEYCONF_PEER_ID, peerid);
...
ovpn_handle_msg(struct nl_msg *msg, void *arg)
{
    ...
    if (nla_parse(attrs, OVPN_A_PEER_MAX, genlmsg_attrdata(gnlh, 0),
                  genlmsg_attrlen(gnlh, 0), NULL))
    {
    ...
    switch (gnlh->cmd)
    {
        case OVPN_CMD_PEER_DEL_NTF:
        {
            ...
            {
                msg(D_DCO, "ovpn-dco: no attributes in OVPN_PEER_DEL message");
                return NL_STOP;
            }

            struct nlattr *dp_attrs[OVPN_A_PEER_MAX + 1];
            if (nla_parse_nested(dp_attrs, OVPN_A_PEER_MAX,
            ...
            {
                msg(D_DCO, "ovpn-dco: no reason in PEER_DEL message");
                return NL_STOP;
            }
            if (!dp_attrs[OVPN_A_PEER_ID])
            {
                msg(D_DCO, "ovpn-dco: no peer-id in PEER_DEL message");
                return NL_STOP;
            }
            int reason = nla_get_u32(dp_attrs[OVPN_A_PEER_DEL_REASON]);
            unsigned int peerid = nla_get_u32(dp_attrs[OVPN_A_PEER_ID]);

            msg(D_DCO_DEBUG, "ovpn-dco: received CMD_PEER_DEL, ifindex: %d, peer-id %d, reason: %d",
        ...
        case OVPN_CMD_PEER_FLOAT_NTF:
        {
            ...
            uint16_t port = nla_get_u16(fp_attrs[OVPN_A_PEER_REMOTE_PORT]);
            struct sockaddr *saddr = &dco->dco_float_peer_sa;

            if (fp_attrs[OVPN_A_PEER_REMOTE_IPV4])
            {
                struct sockaddr_in *ip4 = (struct sockaddr_in *)saddr;
                memcpy(&ip4->sin_addr, nla_data(fp_attrs[OVPN_A_PEER_REMOTE_IPV4]), sizeof(ip4->sin_addr));
            ...
            else if (fp_attrs[OVPN_A_PEER_REMOTE_IPV6])
            {
                ...
                {
                    msg(D_DCO, "ovpn-dco: no scope id in PEER_FLOAT message");
                    return NL_STOP;
                }
                struct sockaddr_in6 *ip6 = (struct sockaddr_in6 *)saddr;
        ...
        case OVPN_CMD_KEY_SWAP_NTF:
        {
            ...
            {
                msg(D_DCO, "ovpn-dco: no attributes in OVPN_KEY_SWAP message");
                return NL_STOP;
            }
            ...
            {
                msg(D_DCO, "received bogus swap key packet data from ovpn-dco");
                return NL_STOP;
            }
            if (!dp_attrs[OVPN_A_KEYCONF_PEER_ID])
            {
                msg(D_DCO, "ovpn-dco: no peer-id in KEY_SWAP message");
                return NL_STOP;
            }
            if (!dp_attrs[OVPN_A_KEYCONF_KEY_ID])
            {
                msg(D_DCO, "ovpn-dco: no key-id in KEY_SWAP message");
                return NL_STOP;
            }
            int key_id = nla_get_u16(dp_attrs[OVPN_A_KEYCONF_KEY_ID]);
            unsigned int peer_id = nla_get_u32(dp_attrs[OVPN_A_KEYCONF_PEER_ID]);

            msg(D_DCO_DEBUG, "ovpn-dco: received CMD_KEY_SWAP, ifindex: %d, peer-id %d, key-id: %d",
