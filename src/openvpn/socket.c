...
create_socket(struct link_socket *sock, struct addrinfo *addr)
{
    ...
    if (sock->bind_dev)
    {
        ...
        else
        {
            ...
            msg(M_INFO, "Using bind-dev %s (ifindex=%u, fib=%d)",
                sock->bind_dev, ifindex, fib);

            if (setsockopt(sock->sd, SOL_SOCKET, SO_SETFIB,
                           &fib, sizeof(fib))
                != 0)
            {
                msg(M_WARN | M_ERRNO,
                    "WARN: setsockopt SO_SETFIB=%d for %s failed",
                    fib, sock->bind_dev);
            }
        }
    }
