         ...
         const bool is_multipoint)
{
    ...
    {
        ret = add_routes(route_list, route_ipv6_list, tt, ROUTE_OPTION_FLAGS(options), es, ctx, is_multipoint);
    ...
    if (force || !(c->sig->signal_received == SIGUSR1 && c->options.persist_tun))
    {
        ...
        if (c->c1.route_list || c->c1.route_ipv6_list)
        {
            ...
            delete_routes(c->c1.route_list, c->c1.route_ipv6_list, c->c1.tuntap,
                          ROUTE_OPTION_FLAGS(&c->options), c->c2.es, &c->net_ctx, c->mode == CM_TOP);
        }
