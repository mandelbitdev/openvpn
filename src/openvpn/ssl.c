...
tls_multi_init_finalize(struct tls_multi *multi, int tls_mtu)
{
    ...
    if (!multi->opt.dco_enabled || (multi->opt.dco_capabilities & DCO_CAP_ASYM_PEER_ID))
    {
...
key_method_2_read(struct buffer *buf, struct tls_multi *multi, struct tls_session *session)
{
    ...
    if (multi->peer_info)
    {
        ...
        {
            /* The peer did not announce an ID or we cannot use asymmetric
             * peer-ids locally: stay symmetric. This must happen before
             * key_method_2_write() so that we do not announce our own ID
             * to the peer either */
