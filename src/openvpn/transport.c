               ...
               struct addrinfo *bind_addresses)
{
    ...
    if (cur)
    {
        indirect = vtab->bind(handle, args, cur->ai_addr, (openvpn_transport_socklen_t)cur->ai_addrlen);
    }
    ...
    else
    {
        ...
        addr_zero_host(&zero);
        indirect = vtab->bind(handle, args, &zero.addr.sa, (openvpn_transport_socklen_t)af_addr_size(ai_family));
    }
