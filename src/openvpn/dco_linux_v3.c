...
ovpn_dco_v3_nlmsg_create(dco_context_t *dco, int cmd)
{
...
int
dco_v3_new_peer(dco_context_t *dco, unsigned int peerid, int sd,
                ...
                struct in_addr *remote_in4, struct in6_addr *remote_in6)
{
    ...
    struct nl_msg *nl_msg = ovpn_dco_v3_nlmsg_create(dco, OVPN_CMD_SET_PEER);
    ...
    if (remoteaddr)
    {
        ...
        NLA_PUT(nl_msg, OVPN_A_PEER_SOCKADDR_REMOTE, alen, remoteaddr);
    }
    ...
    if (localaddr)
    {
        ...
        {
            NLA_PUT(nl_msg, OVPN_A_PEER_LOCAL_IP, sizeof(struct in_addr),
                    &((struct sockaddr_in *)localaddr)->sin_addr);
        }
        else if (localaddr->sa_family == AF_INET6)
        {
            NLA_PUT(nl_msg, OVPN_A_PEER_LOCAL_IP, sizeof(struct in6_addr),
    ...
    if (remote_in4)
    {
        NLA_PUT_U32(nl_msg, OVPN_A_PEER_VPN_IPV4, remote_in4->s_addr);
    }
    if (remote_in6)
    {
...
static int
ovpn_dco_v3_iface_new(struct tuntap *tt, int flags, const char *dev)
{
    int ret = -EMSGSIZE;
    struct nl_msg *nl_msg = nlmsg_alloc();
    genlmsg_put(nl_msg, 0, 0, tt->dco.ovpn_dco_id, 0, flags, OVPN_CMD_NEW_IFACE, 0);
    NLA_PUT(nl_msg, OVPN_A_IFNAME, strlen(dev) + 1, dev);
    NLA_PUT_U32(nl_msg, OVPN_A_MODE, tt->dco.ifmode);

    ret = ovpn_nl_msg_send(&tt->dco, nl_msg, NULL, NULL, __func__);
...
static int
ovpn_dco_v3_iface_del(struct tuntap *tt, const char *dev)
{
    int ret = -EMSGSIZE;
    struct nl_msg *nl_msg = nlmsg_alloc();
    genlmsg_put(nl_msg, 0, 0, tt->dco.ovpn_dco_id, 0, 0, OVPN_CMD_DEL_IFACE, 0);
    if (!tt->dco.ifindex)
    {
        msg(M_FATAL, "DCO: cannot retrieve ifindex for interface %s", dev);
    }

    NLA_PUT_U32(nl_msg, OVPN_A_IFINDEX, tt->dco.ifindex);
...
int
open_tun_dco_v3(struct tuntap *tt, openvpn_net_ctx_t *ctx, const char *dev)
{
    int ret = -EMSGSIZE;
    msg(D_DCO_DEBUG, "%s: %s", __func__, dev);
    ASSERT(tt->type == DEV_TYPE_TUN);

    ret = ovpn_dco_v3_iface_new(tt, 0, dev);
    if (ret < 0)
    {
...
void
close_tun_dco_v3(struct tuntap *tt, openvpn_net_ctx_t *ctx)
{
    msg(D_DCO_DEBUG, __func__);

    ovpn_dco_v3_iface_del(tt, tt->actual_name);

    ovpn_dco_uninit_netlink(&tt->dco);
}

int
dco_v3_swap_keys(dco_context_t *dco, unsigned int peerid)
{
    msg(D_DCO_DEBUG, "%s: peer-id %d", __func__, peerid);

    struct nl_msg *nl_msg = ovpn_dco_v3_nlmsg_create(dco, OVPN_CMD_SWAP_KEYS);
    if (!nl_msg)
    {
        return -ENOMEM;
    }
...
int
dco_v3_del_peer(dco_context_t *dco, unsigned int peerid)
{
    msg(D_DCO_DEBUG, "%s: peer-id %d", __func__, peerid);

    struct nl_msg *nl_msg = ovpn_dco_v3_nlmsg_create(dco, OVPN_CMD_DEL_PEER);
    if (!nl_msg)
    {
        return -ENOMEM;
    }
...
int
dco_v3_del_key(dco_context_t *dco, unsigned int peerid,
               dco_key_slot_t slot)
{
    int ret = -EMSGSIZE;
    msg(D_DCO_DEBUG, "%s: peer-id %d, slot %d", __func__, peerid, slot);

    struct nl_msg *nl_msg = ovpn_dco_v3_nlmsg_create(dco, OVPN_CMD_DEL_KEY);
    if (!nl_msg)
    {
        return -ENOMEM;
    }
    struct nlattr *attr, *keyconf;
    attr = nla_nest_start(nl_msg, OVPN_A_PEER);
    NLA_PUT_U32(nl_msg, OVPN_A_PEER_ID, peerid);

    keyconf = nla_nest_start(nl_msg, OVPN_A_PEER_KEYCONF);
...
int
dco_v3_new_key(dco_context_t *dco, unsigned int peerid, int keyid,
               dco_key_slot_t slot,
               ...
               const char *ciphername)
{
    ...
    struct nl_msg *nl_msg = ovpn_dco_v3_nlmsg_create(dco, OVPN_CMD_SET_KEY);
    if (!nl_msg)
    {
        return -ENOMEM;
    }
    ...
    struct nlattr *key_conf = nla_nest_start(nl_msg, OVPN_A_PEER_KEYCONF);
    NLA_PUT_U32(nl_msg, OVPN_A_KEYCONF_SLOT, slot);
...
int
dco_v3_set_peer(dco_context_t *dco, unsigned int peerid,
...
static int
ovpn_v3_handle_msg(struct nl_msg *msg, void *arg)
{
    ...
    if (nla_parse(attrs, OVPN_A_PEER_MAX + 1, genlmsg_attrdata(gnlh, 0),
    ...
    switch (gnlh->cmd)
    {
        case OVPN_CMD_DEL_PEER:
        {
            ...
            if (nla_parse_nested(dp_attrs, OVPN_A_PEER_MAX + 1,
...
static int
dco_v3_do_read(dco_context_t *dco)
{
    msg(D_DCO_DEBUG, __func__);
    nl_cb_set(dco->nl_cb, NL_CB_VALID, NL_CB_CUSTOM, ovpn_v3_handle_msg, dco);

    return ovpn_nl_recvmsgs(dco, __func__);
}

static void
process_incoming_dco_v3(struct context *c)
{
...
static void
dco_v3_update_peer_stat(struct context_2 *c2, struct nlattr *tb[], uint32_t id)
{
...
int
dco_v3_parse_peer_multi(struct nl_msg *msg, void *arg)
{
    struct nlattr *tb[OVPN_A_PEER_MAX];
    struct genlmsghdr *gnlh = nlmsg_data(nlmsg_hdr(msg));
    ...
    if (!tb[OVPN_A_PEER_ID])
    {
        return NL_SKIP;
    }

    struct nlattr *tb_peer[OVPN_A_PEER_MAX];

    nla_parse(tb_peer, OVPN_A_PEER_MAX,
              nla_data(tb[OVPN_A_PEER_ID]),
              nla_len(tb[OVPN_A_PEER_ID]), NULL);
    ...
    dco_v3_update_peer_stat(&m->instances[peer_id]->context.c2, tb_peer, peer_id);

    return NL_OK;
}

int
dco_v3_get_peer_stats_multi(dco_context_t *dco, struct multi_context *m)
{
    msg(D_DCO_DEBUG, "%s", __func__);

    struct nl_msg *nl_msg = ovpn_dco_v3_nlmsg_create(dco, OVPN_CMD_GET_PEER);

    nlmsg_hdr(nl_msg)->nlmsg_flags |= NLM_F_DUMP;

    int ret = ovpn_nl_msg_send(dco, nl_msg, dco_v3_parse_peer_multi, m, __func__);
...
static int
dco_v3_parse_peer(struct nl_msg *msg, void *arg)
{
    ...
    struct nlattr *tb_peer[OVPN_A_PEER];

    nla_parse(tb_peer, OVPN_A_PEER_ID,
              nla_data(tb[OVPN_A_PEER_ID]),
              nla_len(tb[OVPN_A_PEER_ID]), NULL);
    ...
    dco_v3_update_peer_stat(&c->c2, tb_peer, peer_id);

    return NL_OK;
}

int
dco_v3_get_peer_stats(struct context *c)
{
    ...
    ret = ovpn_nl_msg_send(dco, nl_msg, dco_v3_parse_peer, c, __func__);
...
const struct dco_ops dco_ops_v3 = {
    .family_name = OVPN_FAMILY_NAME,
    .ovpn_dco_nlmsg_create = ovpn_dco_v3_nlmsg_create,
    .dco_new_peer = &dco_v3_new_peer,
    .open_tun_dco = open_tun_dco_v3,
    .close_tun_dco = close_tun_dco_v3,
    .dco_swap_keys = &dco_v3_swap_keys,
    .dco_del_peer = &dco_v3_del_peer,
    .dco_del_key = &dco_v3_del_key,
    .dco_new_key = &dco_v3_new_key,
    .dco_set_peer = &dco_v3_set_peer,
    .ovpn_handle_msg = &ovpn_v3_handle_msg,
    .process_incoming_dco = &process_incoming_dco_v3,
    .dco_do_read = &dco_v3_do_read,
    .dco_update_peer_stat = &dco_v3_update_peer_stat,
    .dco_get_peer_stats_multi = &dco_v3_get_peer_stats_multi,
    .dco_parse_peer = &dco_v3_parse_peer,
    .dco_get_peer_stats = &dco_v3_get_peer_stats
};
