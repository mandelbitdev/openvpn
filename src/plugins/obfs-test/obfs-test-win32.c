                     ...
                     const struct sockaddr *addr, openvpn_transport_socklen_t len)
{
    ...
    free(addr_rev);
    sock->rw_handle->read = &sock->completion_events->read;
    sock->rw_handle->write = &sock->completion_events->write;
...
static struct rw_handle *
obfs_test_win32_get_sock(openvpn_transport_socket_t handle)
{
    struct obfs_test_socket_win32 *sock = (struct obfs_test_socket_win32 *)handle;
...
obfs_test_initialize_vtabs_platform(void)
{
    ...
    obfs_test_socket_vtab.get_sock = obfs_test_win32_get_sock;
